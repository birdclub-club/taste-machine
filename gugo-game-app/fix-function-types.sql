-- Fix the data type mismatch in our leaderboard function
-- Column 14 (win_percentage) is causing "numeric vs double precision" error
...
RETURNS TABLE(
  ...
  win_percentage double precision  -- Changed from numeric to double precision
) AS $$
BEGIN
  RETURN QUERY
  ...
  all_nft_scores AS (
    SELECT 
      ...
      (n.wins::DOUBLE PRECISION / NULLIF(n.total_votes, 0)) * 100 as win_percentage,  -- Ensure DOUBLE PRECISION
  ...
  SELECT 
    ...
    r.win_percentage  -- This should now be DOUBLE PRECISION
  FROM ranked_nfts r
