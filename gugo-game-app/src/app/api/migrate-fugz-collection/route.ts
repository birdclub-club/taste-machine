...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`👽 Starting Fugz migration - batch size: ${batch_size}, offset: ${start_offset}`);

    // Step 1: Get Fugz NFTs with votes
    const { data: fugzNFTs, error: nftError } = await supabase
      .from('nfts')
    ...
    if (nftError) {
      console.error('❌ Error fetching Fugz NFTs:', nftError);
      return NextResponse.json({ success: false, error: nftError.message });
    }

    if (!fugzNFTs || fugzNFTs.length === 0) {
      console.log('✅ No more Fugz NFTs to migrate');
      return NextResponse.json({ 
        success: true, 
        message: 'Fugz migration complete - no more NFTs to process',
        nfts_processed: 0,
        events_migrated: 0
      });
    }

    console.log(`📦 Processing ${fugzNFTs.length} Fugz NFTs...`);
    ...
    console.log(`👽 Fugz batch complete: ${fugzNFTs.length} NFTs → ${totalEvents} events`);

    return NextResponse.json({
      success: true,
      nfts_processed: fugzNFTs.length,
      events_migrated: totalEvents,
      vote_events: voteEvents.length,
      slider_events: sliderEvents.length,
      fire_events: fireEvents.length,
      next_offset: start_offset + fugzNFTs.length
    });

  } catch (error) {
    console.error('❌ Fugz migration error:', error);
    return NextResponse.json({ 
      success: false, 
