...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`🌿 Starting Canna Sapiens migration - batch size: ${batch_size}, offset: ${start_offset}`);

    // Step 1: Get Canna Sapiens NFTs with votes
    const { data: cannaNFTs, error: nftError } = await supabase
      .from('nfts')
      .select('id, name, collection_name, token_id, total_votes, current_elo, slider_average, slider_count')
      .eq('collection_name', 'Canna Sapiens')
    ...
    if (nftError) {
      console.error('❌ Error fetching Canna Sapiens NFTs:', nftError);
      return NextResponse.json({ success: false, error: nftError.message });
    }

    if (!cannaNFTs || cannaNFTs.length === 0) {
      console.log('✅ No more Canna Sapiens NFTs to migrate');
      return NextResponse.json({ 
        success: true, 
        message: 'Canna Sapiens migration complete - no more NFTs to process',
        nfts_processed: 0,
        events_migrated: 0
      });
    }

    console.log(`📦 Processing ${cannaNFTs.length} Canna Sapiens NFTs...`);
    ...
    console.log(`🌿 Canna Sapiens batch complete: ${cannaNFTs.length} NFTs → ${totalEvents} events`);

    return NextResponse.json({
      success: true,
      nfts_processed: cannaNFTs.length,
      events_migrated: totalEvents,
      vote_events: voteEvents.length,
      slider_events: sliderEvents.length,
      fire_events: fireEvents.length,
      next_offset: start_offset + cannaNFTs.length
    });

  } catch (error) {
    console.error('❌ Canna Sapiens migration error:', error);
    return NextResponse.json({ 
      success: false, 
