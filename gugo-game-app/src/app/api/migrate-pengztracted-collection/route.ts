...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`🐧 Starting Pengztracted migration - batch size: ${batch_size}, offset: ${start_offset}`);

    // Step 1: Get Pengztracted NFTs with votes (largest collection by count)
    const { data: pengztractedNFTs, error: nftError } = await supabase
      .from('nfts')
      .select('id, name, collection_name, token_id, total_votes, current_elo, slider_average, slider_count')
      .eq('collection_name', 'Pengztracted')
    ...
    if (nftError) {
      console.error('❌ Error fetching Pengztracted NFTs:', nftError);
      return NextResponse.json({ success: false, error: nftError.message });
    }

    if (!pengztractedNFTs || pengztractedNFTs.length === 0) {
      return NextResponse.json({
        success: true,
        message: 'No more Pengztracted NFTs to migrate',
        migrated: 0,
        total_processed: start_offset
      });
    }

    console.log(`📦 Found ${pengztractedNFTs.length} Pengztracted NFTs to migrate`);

    // Step 2: For each NFT, get its voting history and migrate to events
    let migratedCount = 0;
    const migrationResults = [];

    for (const nft of pengztractedNFTs) {
      try {
    ...
    return NextResponse.json({
      success: true,
      message: `Pengztracted migration batch completed`,
      data: {
        batch_size,
        start_offset,
        nfts_processed: pengztractedNFTs.length,
    ...
    console.error('❌ Pengztracted migration error:', error);
    return NextResponse.json({
      success: false,
