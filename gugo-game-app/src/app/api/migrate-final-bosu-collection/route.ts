...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`🏁 Starting Final Bosu migration - batch size: ${batch_size}, offset: ${start_offset}`);

    // Step 1: Get Final Bosu NFTs with votes
    const { data: finalBosuNFTs, error: nftError } = await supabase
      .from('nfts')
    ...
    if (nftError) {
      console.error('❌ Error fetching Final Bosu NFTs:', nftError);
      return NextResponse.json({ success: false, error: nftError.message });
    }

    if (!finalBosuNFTs || finalBosuNFTs.length === 0) {
      console.log('✅ No more Final Bosu NFTs to migrate');
      return NextResponse.json({ 
        success: true, 
        message: 'Final Bosu migration complete - no more NFTs to process',
        nfts_processed: 0,
        events_migrated: 0
      });
    }

    console.log(`📦 Processing ${finalBosuNFTs.length} Final Bosu NFTs...`);
    ...
    for (const nft of finalBosuNFTs) {
    ...
    console.log(`🏁 Final Bosu batch complete: ${finalBosuNFTs.length} NFTs → ${totalEvents} events`);

    return NextResponse.json({
      success: true,
      nfts_processed: finalBosuNFTs.length,
      events_migrated: totalEvents,
      vote_events: voteEvents.length,
      slider_events: sliderEvents.length,
      fire_events: fireEvents.length,
      next_offset: start_offset + finalBosuNFTs.length
    });

  } catch (error) {
    console.error('❌ Final Bosu migration error:', error);
    return NextResponse.json({ 
      success: false, 
