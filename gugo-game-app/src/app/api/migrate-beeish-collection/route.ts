...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`🐝 Starting BEEISH migration - batch size: ${batch_size}, offset: ${start_offset}`);

    // Step 1: Get BEEISH NFTs with votes (our most active collection)
    const { data: beeishNFTs, error: nftError } = await supabase
      .from('nfts')
    ...
    if (nftError) {
      console.error('❌ Error fetching BEEISH NFTs:', nftError);
      return NextResponse.json({ success: false, error: nftError.message });
    }

    if (!beeishNFTs || beeishNFTs.length === 0) {
      return NextResponse.json({
        success: true,
        message: 'No more BEEISH NFTs to migrate',
        migrated: 0,
        total_processed: start_offset
      });
    }

    console.log(`📦 Found ${beeishNFTs.length} BEEISH NFTs to migrate`);

    // Step 2: For each NFT, get its voting history and migrate to events
    let migratedCount = 0;
    const migrationResults = [];

    for (const nft of beeishNFTs) {
      try {
    ...
    return NextResponse.json({
      success: true,
      message: `BEEISH migration batch completed`,
      data: {
        batch_size,
        start_offset,
        nfts_processed: beeishNFTs.length,
    ...
    console.error('❌ BEEISH migration error:', error);
    return NextResponse.json({
      success: false,
