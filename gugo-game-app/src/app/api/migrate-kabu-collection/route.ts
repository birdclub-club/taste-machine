...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`🎋 Starting Kabu migration - batch size: ${batch_size}, offset: ${start_offset}`);

    // Step 1: Get Kabu NFTs with votes (fourth priority collection)
    const { data: kabuNFTs, error: nftError } = await supabase
      .from('nfts')
    ...
    if (nftError) {
      console.error('❌ Error fetching Kabu NFTs:', nftError);
      return NextResponse.json({ success: false, error: nftError.message });
    }

    if (!kabuNFTs || kabuNFTs.length === 0) {
      return NextResponse.json({
        success: true,
        message: 'No more Kabu NFTs to migrate',
        migrated: 0,
        total_processed: start_offset
      });
    }

    console.log(`📦 Found ${kabuNFTs.length} Kabu NFTs to migrate`);

    // Step 2: For each NFT, get its voting history and migrate to events
    let migratedCount = 0;
    const migrationResults = [];

    for (const nft of kabuNFTs) {
      try {
    ...
    return NextResponse.json({
      success: true,
      message: `Kabu migration batch completed`,
      data: {
        batch_size,
        start_offset,
        nfts_processed: kabuNFTs.length,
    ...
    console.error('❌ Kabu migration error:', error);
    return NextResponse.json({
      success: false,
