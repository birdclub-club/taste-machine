...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`🎨 Starting DreamilioMaker migration - batch size: ${batch_size}, offset: ${start_offset}`);

    // Step 1: Get DreamilioMaker NFTs with votes (final priority collection)
    const { data: dreamilioNFTs, error: nftError } = await supabase
      .from('nfts')
    ...
    if (nftError) {
      console.error('❌ Error fetching DreamilioMaker NFTs:', nftError);
      return NextResponse.json({ success: false, error: nftError.message });
    }

    if (!dreamilioNFTs || dreamilioNFTs.length === 0) {
      console.log('✅ No more DreamilioMaker NFTs to migrate');
      return NextResponse.json({ 
        success: true, 
        message: 'DreamilioMaker migration complete - no more NFTs to process',
        nfts_processed: 0,
        events_migrated: 0
      });
    }

    console.log(`📦 Processing ${dreamilioNFTs.length} DreamilioMaker NFTs...`);
    ...
    for (const nft of dreamilioNFTs) {
    ...
    console.log(`🎨 DreamilioMaker batch complete: ${dreamilioNFTs.length} NFTs → ${totalEvents} events`);

    return NextResponse.json({
      success: true,
      nfts_processed: dreamilioNFTs.length,
      events_migrated: totalEvents,
      vote_events: voteEvents.length,
      slider_events: sliderEvents.length,
      fire_events: fireEvents.length,
      next_offset: start_offset + dreamilioNFTs.length
    });

  } catch (error) {
    console.error('❌ DreamilioMaker migration error:', error);
    return NextResponse.json({ 
      success: false, 
