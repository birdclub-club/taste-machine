-- Fix ambiguous column reference in fire_votes
-- This resolves the "column reference 'fire_votes' is ambiguous" error
...
BEGIN
  ...
  all_nft_scores AS (
    SELECT 
      ...
      n.wins,
      n.losses,
      COALESCE(f.fire_count, 0) as nft_fire_votes,  -- Changed name to avoid ambiguity
  ...
  ranked_nfts AS (
    SELECT 
      *,
      ROW_NUMBER() OVER (
        ORDER BY 
          nft_fire_votes DESC,  -- Use the renamed column
          poa_score DESC,
          total_votes DESC,
          RANDOM()
