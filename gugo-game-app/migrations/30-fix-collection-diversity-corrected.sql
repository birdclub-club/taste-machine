-- Fix Collection Diversity in Dynamic POA System (Corrected UNION syntax)
...
BEGIN
  ...
  -- Get overall top performers separately
  overall_top AS (
    SELECT *, 1 as collection_rank
    FROM nft_poa_scores 
  ...
  diverse_candidates AS (
    ...
    SELECT 
      nft_id, nft_token_id, nft_name, nft_image, nft_collection_name, nft_contract_address,
      nft_current_elo, nft_slider_average, nft_slider_count, nft_total_votes, nft_wins, nft_losses,
      nft_fire_votes, calculated_poa_score, calculated_confidence_score, calculated_confidence_level,
      calculated_algorithm, calculated_system_mode, calculated_win_percentage, collection_rank
    FROM top_per_collection WHERE collection_rank <= 2
    UNION ALL
  ...
  final_ranking AS (
    SELECT DISTINCT ON (nft_id)
      ...
      nft_current_elo, nft_slider_average, nft_slider_count, nft_total_votes, nft_wins, nft_losses,
      nft_fire_votes, calculated_poa_score, calculated_confidence_score, calculated_confidence_level,
      calculated_algorithm, calculated_system_mode, calculated_win_percentage
